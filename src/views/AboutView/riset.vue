...
import { faPlay, faPause, faStepForward, faStepBackward, faShuffle, faRepeat, faVolumeLow, faVolumeXmark, faTerminal } from '@fortawesome/free-solid-svg-icons';

// Utils
import { useEventListener } from '@/utils/eventListener';
import { useAudioPlayer } from '@/utils/composables/useAudioPlayer';
...
const commandsList = {
  ...
  ls: (args) => {
    if (args === 'songs') {
      return store.songs.map((item, index) => `${index + 1}. ${item.title}`)
    } else if (args === '') {
      const result = `
                        <span>
  ...
  play: (args) => {
    if (args === '') {
      return `Please pick one song. Check with command <span class='text-gray-100'>'ls songs'</span> to list down the available songs.`
    } else if (typeof parseInt(args) === 'number') {
      let indexArgs = parseInt(args)
      let selectedSong = store.songs[indexArgs - 1]
      if (selectedSong) {
        handleChangeSong(indexArgs - 1)
        return `<marquee>Playing ${currentSong.value.title}</marquee>`
      } else return 'There is no songs. Try pick another!'
    }
  }
}
...
useEventListener(window, 'keydown', true, (e) => {
  ...
  if (!isTyping && e.code === 'Space') handleRunTrack(store.activeIndex);
});
...
onMounted(() => {
  ...
  useEventListener(audio.value, 'ended', false, function () {
    ...
    handleChangeSong(store.activeIndex);
  });
});
...
<template>
  <ModalDraggable :is-open="showTerminal">
    <Terminal2 title-header="Skuytify" :commands="commandsList" @handle-close="toggleShowTerminal(false)" />
  </ModalDraggable>
    ...
    <div class="h-[calc(100%-5.5rem)] p-2 w-full bg-backgroundSecondary rounded-lg flex flex-col gap-y-2">
      <div class="flex flex-col gap-2 p-1">
        <div class="flex flex-row items-center gap-5">
          <h1 class="text-xl font-semibold dark:text-whiteApp">Recently Plays</h1>
          <!-- Icon Terminal -->
          <div class="w-[35px] flex items-center p-2 border border-textColor rounded-md cursor-pointer"
            @click.prevent="toggleShowTerminal(true)">
            <FontAwesomeIcon :icon="faTerminal" class="text-xs text-textColor dark:text-whiteApp" />
          </div>
        </div>
